...
type ProductState = "delete" | "deduct" | "edit" | null;
...
const ProductDetailModal: FC<ProductDetailModalProps> = ({
  ...
  const [dialog, setDialog] = useState<ProductState>(null);
    ...
    delete: {
      ...
      onConfirm: async () => {
        try {
          ...
          console.log(error);
          setDialog(null);
          setToast({ variant: "danger", message: "Something went wrong!" });
        }
      },
    },
    deduct: {
  ...
  return (
    <>
      <Modal isOpen={isOpen} title={product.name} onClose={handleCloseAll}>
        <div className="w-2xl">
          {/* Category & Price */}
            ...
            <div className="flex justify-end items-center w-1/2 gap-2">
              ...
              <button
                ref={(node) => {
                  if (node)
                    node.disabled
                      ? node.classList.add("button-muted")
                      : node.classList.add("button-accent");
                }}
      ...
      {toast && (
        <Toast
          isOpen
          forceToTop={toast.forceToTop ?? true}
